...
def move(lines, cost, x, y, dx, dy, X, Y, min_steps=0, max_steps=3):
...
def main(path):
    ...
    while len(beam) != 0:
        ...
        if x == X-1 and y == Y-1:
            break
        if d == 0 or d % 2 == 0:
            for prio2, cost2, x2, y2 in move(lines, cost, x, y, +1, 0, X, Y, 4, 10):
                heapq.heappush(beam, (prio2, cost2, x2, y2, 1))
            for prio2, cost2, x2, y2 in move(lines, cost, x, y, -1, 0, X, Y, 4, 10):
                heapq.heappush(beam, (prio2, cost2, x2, y2, 1))
        if d == 0 or d % 2 == 1:
            for prio2, cost2, x2, y2 in move(lines, cost, x, y, 0, +1, X, Y, 4, 10):
